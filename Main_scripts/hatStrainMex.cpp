...
/*    
 *   hatStrainMex.cpp
 *   Calculate hat strain field for 3 noded triangular element
 *   15 June 2017
*/

// Compile Command for Windows 
// mex hatStrainMex.cpp COMPFLAGS="/openmp $COMPFLAGS"

// Compile Command for Mac 
// 

void hatStrain(double *, int *, int , double *, double *, double *, int, int , double *, double *, double *, double *);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	for (i=0; i<npts; i++)
	{
		hatStrain(uhat, ncC, mel, xnode, ynode, DC, elegrainindex[belong[i]-1], belong[i]-1, &s11[i], &s22[i], &s12[i], &s21[i]);
	}
}

void hatStrain(double *uhat, int *ncC, int mel, double *xnode, double *ynode, double *DC, int elegrainindexL, int belongL, double *s11, double *s22, double *s12, double *s21)
{
    double Be[3][6]={0};    
    double DCL[9]={0};
	double U[6]={0};
