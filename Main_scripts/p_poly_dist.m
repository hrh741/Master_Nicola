...
% if finding whether the points are inside or outsite the polygon is
...
[dpv_min, I_dpv_min] = min(abs(dpv),[],2); % I_dpv_min = index at which the min. value is placed
% min(A,[],2) is a column vector containing the minimum value of each row.

% coordinates of consecutive vertices
P1 = Pv(1:(end-1),:); % 4x2 neglecting last row
P2 = Pv(2:end,:); % 4x2 neglecting Ist row
dv = P2 - P1;
